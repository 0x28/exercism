...
specs = describe "value" $ do
  ...
  describe "property tests" $ do
    ...
    it "all values not starting with Black are two digits" $
      forAll colorGen2 (\(a, b) -> a /= Black ==> value (a, b) >= 10)

    it "reversing the arguments reverses the result" $
      forAll colorGen2 (\(a, b) -> a /= Black && b /= Black ==>
                          show (value (a,b)) == reverse (show $ value (b, a)))

    it "all color names are different" $
      forAll colorGen2 (\(a,b) -> (a /= b) == (show a /= show b))
